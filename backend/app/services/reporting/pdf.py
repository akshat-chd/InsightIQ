...
__all__ = ["build_pdf", "build_generic_pdf"]
...
def build_generic_pdf(
    ...
    *,
    ...
    """Build a simple ReportLab PDF for a Generic Dataset Analysis."""
    styles = _styles()
    buf = io.BytesIO()
    doc = SimpleDocTemplate(
        ...
        topMargin=MARGIN,
        bottomMargin=MARGIN,
        title=f"InsightIQ Report: {title}",
        author="InsightIQ",
    )
    ...
    story.append(Paragraph(
        f"{organization_name} &bull; Generated {datetime.now(UTC).strftime('%d %b %Y, %H:%M UTC')}",
        styles["subtitle"],
    ))
    ...
    story.append(Paragraph(f"Rows: {result.get('rows', 0):,} | Columns: {len(result.get('columns', []))}", styles["body"]))
    story.append(Spacer(1, 12))
    
    # Stats table
    if result.get("stats"):
        story.append(Paragraph("Numeric Column Statistics", styles["h3"]))
        table_data = [["Column", "Mean", "Min", "Max", "Missing"]]
        ...
        table = Table(table_data, colWidths=[50*mm, 25*mm, 25*mm, 25*mm, 25*mm])
    ...
    if include_narrative and result.get("narrative"):
        ...
        blocks = to_blocks(result["narrative"])
        for blk in to_reportlab(blocks, styles):
            story.append(blk)
