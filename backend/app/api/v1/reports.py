...
from app.schemas.report import GenericReportRequest
@router.post(
    "/export-generic",
    dependencies=[require(Permission.REPORT_EXPORT)],
    summary="Export a generic dataset analysis as PDF",
    response_class=Response,
)
async def export_generic(
    payload: GenericReportRequest, auth: CurrentAuth, session: DbSession
) -> Response:
    service = ReportService(session, auth.organization_id)
    report = await service.generate_generic(
        payload,
        organization_name=auth.organization.name,
        user_id=auth.user.id,
        actor_email=auth.user.email,
